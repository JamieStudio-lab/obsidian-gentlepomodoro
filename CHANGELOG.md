...
- **A next-video button for playlists.** Press ⏩ to skip to the next video in the playlist you're listening to — it fades out and back in like the other controls rather than cutting, and pressing ⏸ or ⏹ while it fades cancels the skip. It appears only when the current link is actually a playlist, so it stays out of the way for ordinary videos — including looped ones, which YouTube internally treats as a one-item playlist.
