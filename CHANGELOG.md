...
- **Up to three music links, switchable from the timer panel.** Settings now has three link slots instead of one, each with an optional short name ("Lofi", "Rain"). The music controls now open with a box naming what is playing — tap it, or the new list button, to pick a different link. Leave the extra slots empty and there is simply nothing to pick from. Your existing link becomes the first slot automatically — nothing to set up, and nothing is lost.
...
- **A next-video button for playlists.** Press ⏩ to skip to the next video in the playlist you're listening to. It appears only when the current link is actually a playlist, so it stays out of the way for ordinary videos — including looped ones, which YouTube internally treats as a one-item playlist.
...
- **Links name themselves.** Paste a link and the name box beside it fills in on its own — the channel name for a video, the playlist's title for a playlist. You can still type over it, and it never touches a name you have already written, or one you are in the middle of writing. (A video's _title_ is deliberately not used: they run to advertising copy, and some are unusable — one link tested here has a title that is a single invisible character.)
...
- Two older bugs surfaced while building this and are fixed here: pressing ⏹ and then switching links straight away made the new link start from the top and forget where it had been; and switching links while music played, then pressing ▶️, skipped the fade and started at full volume.
