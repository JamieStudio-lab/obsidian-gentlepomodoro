...
- **Up to three links, switchable from the panel**: fill in a second or third link (each with an optional short name like _Lofi_ or _Rain_) and you can change the mood without opening settings. The controls open with a box naming what's playing; tap it — or the list button beside the transport — to pick another. Picking one only selects it: the player reloads and lands silent, so press ▶️ to start. Renaming a link, or editing one you aren't listening to, never interrupts what's playing.
...
- **⏩ next video** skips ahead in a playlist. Shown only when the link you're on is actually a playlist.
- **Links check themselves and name themselves**: paste a link and, if YouTube has no record of it — a typo, a deleted or private video, one that blocks embedding — a short message appears under the box in settings. The name box fills in on its own too (the channel for a video, the playlist's title for a playlist); type over it any time, and it never touches a name you've already written. The check is honest about its limits: it can't promise a link will play, only that YouTube can find it.
