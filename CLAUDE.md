...
- **`obsidian` dep is pinned to `^1.4.16`** (types-only npm package). [package.json](package.json) and [manifest.json](manifest.json) track 0.3.0; [versions.json](versions.json) maps each plugin version to its `minAppVersion`.
...
- **iPad icon sizing is forced inline, not via CSS.** WebKit/Safari collapses an SVG flex item's main-axis (width) to ~min-content, so CSS `width` / `!important` on `.gp-icon-btn svg` does **not** hold on iPad (the glyph renders ~8px wide × full height). `GentlePomoView.onOpen` pins width/height **plus a `min-width`/`min-height` floor** inline with `!important` (the eslint-disabled `style.setProperty` block, value from `--gp-icon-svg-size`). The `min-*` floor is the load-bearing part — the flex algorithm can't shrink below it. Don't convert this to a CSS class or drop the `min-width`; it regresses the iPad glyphs to thin slivers. (Diagnosed device-side via Safari Web Inspector; desktop/iPhone were never affected.)
