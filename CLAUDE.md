...
**Gentle Pomodoro** is an Obsidian plugin: a soothing, task-integrated Pomodoro timer that links sessions to Tasks-plugin markdown items and writes Dataview-compatible daily logs. Currently **v0.3.0 (beta)**.
...
- **`obsidian` dep is pinned to `^1.4.16`** (types-only npm package). [package.json](package.json) and [manifest.json](manifest.json) track 0.3.0; `minAppVersion` is **1.7.2** (raised from 1.0.0 for `Workspace.revealLeaf` / `Vault.createFolder`, which `eslint-plugin-obsidianmd`'s `no-unsupported-api` rule gates). [versions.json](versions.json) maps each plugin version to its `minAppVersion`.
